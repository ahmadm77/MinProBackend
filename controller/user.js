...
const userController = {
  register : async (req, res) => {
    try {
      ...
      if (d == null) {
        ...
        const token = jwt.sign({ username, email, phonenumber, password }, 'ahmad123', {expiresIn :'1h'});
      
        await transporter.sendMail({
          from: 'ahmad77mawardi@gmail.com',
          to: email,
          subject :'test',
          html : '<h1>Sukses</h1>'
        });
  ...
  login : async (req, res)=> {
    try {
      ...
      const token = jwt.sign(payLoad, 'ahmad123');
      res.status(200).send({
        message : 'Success',
        data,
        token : token
  ...
  editEmail : async (req, res)=>{
    try {
      ...
      if (isEmailExist == null) {
        ...
        const token = jwt.sign({username, email, newEmail, phone }, 'ahmad123', {expiresIn :'1h'});
        await transporter.sendMail({
